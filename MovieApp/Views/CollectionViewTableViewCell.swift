...
//  MovieApp
//
//  Created by Hakan Körhasan on 27.08.2022.
//
...
class CollectionViewTableViewCell: UITableViewCell {
    ...
    private func favoriteTitleAt(indexPath: IndexPath){
        ...
        CoreDataPersistanceManager.shared.favoriteTitleWith(model: titles[indexPath.row]) { result in
            switch result {
            case .success():
...
extension CollectionViewTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        NetworkingManager.shared.getMovieTrailer(with: titleName + " trailer") { [weak self] result in
            switch result {
            case .success(let videoElement):
                ...
                let viewModel = TitlePreviewViewModels(title: titleName, youtubeView: videoElement, titleOverview: titleOverview)
